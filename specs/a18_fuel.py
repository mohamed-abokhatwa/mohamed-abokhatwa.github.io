...
<div class="fig">
  <div class="fig-head">
    ...
    <div class="fsub">NFPA 20 base tank = 5.07 L per rated hp, plus 5 % for expansion and sump. Run time from the engine&rsquo;s consumption at full load.</div>
  </div>
  ...
  <div class="readout">
    <div class="cell"><div class="k">NFPA 20 base tank</div><div class="v" id="rFb">1,597 <small>L</small></div></div>
    <div class="cell"><div class="k">Run time on base tank</div><div class="v" id="rFt">25.3 <small>h</small></div></div>
    <div class="cell"><div class="k">Needed for run time</div><div class="v" id="rFn2">504 <small>L</small></div></div>
    <div class="cell"><div class="k">Total, all sets</div><div class="v" id="rFa">6,388 <small>L</small></div></div>
    <div class="cell"><div class="k">Governing</div><div class="v" style="font-size:15px;margin-top:6px;"><span id="rFv"></span></div></div>
  </div>
</div>
<p class="fig-note">A 300&nbsp;hp fire pump driver takes a <strong>1,597&nbsp;L</strong> base tank under the NFPA&nbsp;20 rule, which at 0.21&nbsp;L/hp·h is over <strong>25 hours</strong> of running — far more than the eight hours the fire strategy asks for. That is deliberate: the rule is a prescriptive minimum designed to remove judgement, and in a zoned megatall with a pump set per zone it means <strong>four separate dedicated tanks totalling 6,400&nbsp;L</strong>, each with its own bund, fill point, level monitoring and weekly test regime. Do not attempt to consolidate them into one tank serving several pump rooms — the whole point of the rule is that each set is independent of every other system in the building.</p>
...
const NFPA_L_PER_HP=5.07, NFPA_MARGIN=1.05;
