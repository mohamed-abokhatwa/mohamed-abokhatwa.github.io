...
<div class="fig">
  <div class="fig-head">
    ...
    <div class="fsub">h = 1.006·T + W(2501 + 1.86·T), with W from temperature and relative humidity. Recovered load = ρ·Q·Δh·ε against exhaust air at 24 °C / 50 % RH.</div>
  </div>
  ...
  <div class="readout">
    ...
    <div class="cell"><div class="k">Untreated OA load</div><div class="v" id="rL0">712 <small>kW</small></div></div>
    <div class="cell"><div class="k">Recovered</div><div class="v" id="rRe">498 <small>kW</small></div></div>
    <div class="cell"><div class="k">Chiller saved</div><div class="v" id="rTr">142 <small>TR</small></div></div>
  </div>
</div>
<p class="fig-note">The regional split is stark. At <strong>Jeddah</strong> conditions (40&nbsp;°C, 55&nbsp;%) the outdoor air carries 107&nbsp;kJ/kg against 48&nbsp;kJ/kg leaving the building — a <strong>59&nbsp;kJ/kg</strong> gap, so 10&nbsp;m³/s of fresh air is a 712&nbsp;kW load and a 70&nbsp;% wheel recovers <strong>498&nbsp;kW</strong>, about 142 tons of chiller you never have to buy or run. Drag the humidity down to 15&nbsp;% for <strong>Riyadh</strong> and the gap collapses to about 18&nbsp;kJ/kg and the recovery to 155&nbsp;kW. Same building, same wheel, a third of the benefit — which is why energy recovery is close to mandatory on the coast and a genuine cost-benefit question inland. Note also what this says about leakage: in Jeddah every extra litre of uncontrolled infiltration costs about three times what it costs in Riyadh.</p>
...
<div class="callout key">
  ...
  At 600&nbsp;m an ordinary day produces <strong>600&nbsp;Pa across the building</strong> — more than the fan's own static — so intake location, damper selection and plenum separation are ventilation design, not architectural detailing. Recover the outdoor air's enthalpy where the climate is humid, because on the coast fresh air is most of the latent load and a wheel is worth a third of your chiller; specify filters on depth rather than class, because their pressure drop runs all year; and separate ventilation from cooling so the riser carries a tenth of the air and the fresh air is something you can actually measure.
</div>
...
function updErv(){
  ...
  const load=r=>Math.max(0,RHO*Q*(enth(T,humRatio(T,r/100))-H_RET));
  ...
  const ho=enth(T,humRatio(T,rh)), dh=ho-H_RET, L0=Math.max(0,RHO*Q*dh), rec=L0*ef;
