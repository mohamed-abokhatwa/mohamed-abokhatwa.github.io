...
<p>Each bar shows what fills the shell at the lowest pressure: expanded gas, and the water still inside. The gas at the minimum is the closed form, so at the reference duty the air-over-water shell reads 19.8&nbsp;m&sup3; rather than the transient model's 19.1&nbsp;m&sup3;, which is drawn dashed while the duty controls sit at the reference values.</p>

<div class="fig">
  <div class="fig-head">
    <div class="ftitle">Shell volume for the same gas duty: air-over-water vs bladder or diaphragm</div>
    <div class="fsub">Gas at the minimum from polytropic expansion. Bladder shell from isothermal charging to the steady HGL. Air-over-water shell = gas at the minimum / (1 &minus; reserve). Pressures absolute, atmosphere 10.33 m. Dashed line: transient-model air-over-water shell, shown only at the reference duty.</div>
  </div>
...
<div class="fig">
  ...
  <div class="readout">
    ...
    <div class="cell"><div class="k">Line minimum</div><div class="v" id="rLmin4">+4.3 <small>m</small></div></div>
    <div class="cell"><div class="k">Line maximum</div><div class="v" id="rLmax4">119.2 <small>m</small></div></div>
...
<p class="fig-note">At the design setting the line minimum is <strong>+4.3&nbsp;m</strong> and the gas expands to 16.17&nbsp;m&sup3;, leaving 3.83&nbsp;m&sup3; of water. Select 2.00&nbsp;m&sup3;, a compressor that has fallen behind dissolution, and the minimum drops to <strong>&minus;3.8&nbsp;m</strong>: below the criterion, still clear of vapour. Select 4.50&nbsp;m&sup3; and the minimum improves to +7.2&nbsp;m, but only 1.52&nbsp;m&sup3; of water is left at maximum expansion. Select 6.00&nbsp;m&sup3;, a transmitter reading high, and the vessel empties: vapour forms at the pump end and a real vessel would pass air into the main (see <a href="air-admission-networks.html">air admission</a>). The later maximum of about 96&nbsp;m comes from the returning column compressing the gas, about a minute after the cavity has closed, which is why both cavity models agree. The failures sit either side of the design setting, so an air-over-water vessel needs both a low-gas and a high-gas alarm, and both drift cases belong in the HAMMER study.</p>
...
const sgn1=v=>(v>=0?'+':'&minus;')+fmt1(Math.abs(v));
const sgn2=v=>(v>=0?'+':'&minus;')+fmt2(Math.abs(v));
...
function updShell(){
  ...
  document.getElementById('rPen').innerHTML=d.pen>=0
    ? '<span class="badge warn">+'+fmt0(d.pen)+' %</span>'
    : '<span class="badge good">&minus;'+fmt0(-d.pen)+' %</span>';
  const g=d.Ppre-HBAR;
